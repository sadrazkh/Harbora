...
public static class FunctionProject
{
    ...
    private static string CSharpProgram(IReadOnlyList<FunctionDefinition> fns, int port)
    {
        ...
        return
            ...
            app.MapPost("{{InvokePathPrefix}}{slug}", async (string slug, HttpRequest http) =>
            {
                ...
                return await RunAsync(fn, request, ctx);
            });
            ...
            // without being handed a stack trace.
            static async Task<IResult> RunAsync(Registration fn, FnRequest request, FnContext ctx)
            {
                if (!fn.Enabled)
                ...
                try
                {
                    ...
                    Console.WriteLine($"[{fn.Slug}] {ctx.Trigger} {response.Status} {Stopwatch.GetElapsedTime(started).TotalMilliseconds:0}ms");
                    return Results.Content(response.Body, ContentType(response), statusCode: response.Status);
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"[{fn.Slug}] {ctx.Trigger} FAILED after {Stopwatch.GetElapsedTime(started).TotalMilliseconds:0}ms: {ex}");
    ...
    private static string JavaScriptServer(IReadOnlyList<FunctionDefinition> fns, int port)
    {
        ...
        return
            ...
            async function run(fn, request, ctx, res) {
              if (!fn.enabled) { send(res, { status: 503, headers: {}, body: '' }); return; }
              const started = Date.now();
              try {
                const out = normalise(await fn.handler(request, ctx));
                console.log(`[${fn.slug}] ${ctx.trigger} ${out.status} ${Date.now() - started}ms`);
                send(res, out);
              } catch (err) {
                console.log(`[${fn.slug}] ${ctx.trigger} FAILED after ${Date.now() - started}ms: ${err && err.stack || err}`);
            ...
            createServer(async (req, res) => {
              ...
              if (path.startsWith('{{InvokePathPrefix}}')) {
                ...
                await run(fn, request, context(fn.slug, envelope.trigger || 'event', envelope.event), res);
                return;
              }
              ...
              await run(fn, request, context(fn.slug, 'http'), res);
            ...
            class Handler(BaseHTTPRequestHandler):
                ...
                def _run(self, fn, request, ctx):
                    ...
                    try:
                        ...
                        print('[%s] %s %s %dms' % (fn['slug'], ctx['trigger'], status, (time.time() - started) * 1000))
                        self._send(status, headers, body)
                    except Exception:
                        print('[%s] %s FAILED after %dms:\n%s' % (
                            fn['slug'], ctx['trigger'], (time.time() - started) * 1000, traceback.format_exc()))
                ...
                def _dispatch(self):
                    ...
                    self._run(fn, request, context(fn['slug'], 'http'))
