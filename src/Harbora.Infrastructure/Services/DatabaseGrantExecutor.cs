    ...
    ILogger<DatabaseGrantExecutor> logger)
{
    ...
    /// <summary>
    /// Installs pgvector inside a logical database (1.7, pgvector-as-option plan) — the same one-off
    /// seam every statement in this class already runs through, reused rather than opened a second
    /// time. Returns <c>Answered</c> for the same reason <see cref="RotateAsync"/> does: a caller that
    /// wants to record whether the extension is now present has to be able to tell "the engine said
    /// no" from "nothing was risked, so nothing is known" — writing the latter as a plain false would
    /// tell a customer their extension disappeared when it may still be sitting there untouched.
    /// </summary>
    public Task<(bool Ok, string? Error, bool Answered)> CreateVectorExtensionAsync(
        ManagedService service, string networkName, string database, CancellationToken ct)
    {
        if (!DatabaseGrantSql.SupportsVectorExtension(service.Type))
            return Task.FromResult((false, (string?)DatabaseGrantSql.VectorExtensionUnsupportedReason(service.Type), true));

        var command = DatabaseGrantSql.CreateVectorExtension(
            service.Type, service.ContainerName, service.InternalPort, service.Username, database);

        // Null means a value failed the safety check — everything here is generated by Harbora, so
        // this is a bug or an attack, not a real refusal from the engine. Nothing was run, so the
        // answer is as certain as it gets, the same reasoning RotateAsync's own null-command arm uses.
        if (command is null)
            return Task.FromResult((false, (string?)"That database's name cannot be used in a statement.", true));
