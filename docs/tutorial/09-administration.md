...
### دعوت با ایمیل (به‌جای گفتنِ رمز)

اگر SMTP تنظیم باشد، زیر همان فرم گزینهٔ **«یا دعوت با ایمیل»** هست. حساب با رمزی تصادفی ساخته
می‌شود که **هیچ‌کس نمی‌داند**، و تنها راه ورود، لینک گذاشتنِ رمز در ایمیل خود آن شخص است — یک‌بار
مصرف و یک‌ساعته. رمز موقتی که در چت گفته می‌شود، در چت هم می‌ماند.

### بازنشانی ۲مرحله‌ای

اگر کسی هم گوشی و هم کدهای بازیابی‌اش را از دست داد، دکمهٔ **بازنشانی ۲مرحله‌ای** کنار همان ردیف
لایهٔ دوم را برمی‌دارد تا با رمز وارد شود. کار مدیر است و در گزارش رخدادها ثبت می‌شود.
...
کنار کارت مصرف، **خروجی CSV همهٔ ماه‌ها** — همان ارقام برای صورتحساب یا صفحهٔ گسترده، به‌جای
رونویسی دستی.

## ایمیل خروجی (SMTP)

در **تنظیمات پلتفرم** › **ایمیل خروجی**. یک حساب برای کل پلتفرم: بازنشانی رمز، دعوت اعضا، و
پشتیبانِ کانال‌های هشداری که سرور خودشان را ننوشته‌اند.

* رمز مثل هر اعتبار دیگری رمزنگاری‌شده ذخیره می‌شود و دیگر نشان داده نمی‌شود؛ برای تغییرِ فقط
  میزبان یا پورت، فیلد رمز را خالی بگذارید.
* **ارسال ایمیل آزمایشی** از همان مسیری می‌رود که هر ایمیل واقعی می‌رود، و اگر سرور «نه» بگوید،
  همان جملهٔ سرور را نشان می‌دهد. تستی که نمی‌تواند شکست بخورد چیزی را تست نمی‌کند.
* تا وقتی SMTP تنظیم نشده، لینک فراموشی رمز و گزینهٔ دعوت اصلاً نمایش داده نمی‌شوند.

## بررسی به‌روزرسانی

در **تنظیمات پلتفرم** › **به‌روزرسانی Harbora**. **پیش‌فرض خاموش** است، چون یک درخواست خروجی به
شخص ثالث (GitHub) است و بررسی‌ای که کسی نخواسته، تله‌متری است.

روشن که باشد، روزی یک‌بار آخرین نسخهٔ منتشرشده را می‌خواند. اگر واقعاً جلوتر از نسخهٔ در حال اجرا
بود، روی داشبورد — فقط برای مالک/مدیر — یک خط اطلاعی می‌آید. **هیچ‌چیز خودکار نصب نمی‌شود**؛
به‌روزرسانی روی سرور انجام می‌شود.
