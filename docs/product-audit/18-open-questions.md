...
**Q15 — What should a workspace-scoped operator see in the audit log?** (0056) — **ANSWERED, 2026-08-27.**
`AuditController` reads `AuditLogs` unscoped. The table is deliberately unfiltered — that is right for the writer, since most rows are platform events belonging to no workspace — but the reader is a page a tenant can open. Options: (a) provider-only page, hidden from workspace roles entirely; (b) scoped to the caller's workspace, which hides the platform events that are most of the table and makes the page look broken; (c) scoped for workspace roles, unscoped for the provider, with the page saying which view you are looking at. **The default, (c), is taken and built.** `AuditLog` gained a nullable `WorkspaceId` (no ambient query filter — same reasoning as `Job`/`NotificationDelivery` — so every reader filters explicitly); `IAuditLogger.LogAsync` gained a matching parameter that every call site now sets deliberately (a real workspace, or an explicit `null` for a platform-level act) rather than the sink defaulting it from the caller's own session. The platform-wide `AuditController` is unchanged. The workspace-scoped reader is new: `WorkspacesController.AuditLog` at `/workspaces/audit-log`, explicitly `WorkspaceId ==`-filtered, which states in the page itself that platform-level events are excluded by design rather than implying a complete history. Pre-existing rows and platform-level rows both read as absent from a workspace's own list — never hidden behind a lie, never fabricated into a workspace they do not belong to. Tenancy proven both directions (plus the platform-level-row case) in `WorkspaceAuditLogPageHttpTests`.
