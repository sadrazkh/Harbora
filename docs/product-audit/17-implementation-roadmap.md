...
For the Phase 1/2 repository closeout, implementation, repository validation, and external/live
validation are reported separately. A complete implementation does not claim that a VPS drill ran;
conversely, an infrastructure-dependent drill that is explicitly assigned to the external follow-up
does not keep the implementation branch open forever. Any defect found by that follow-up reopens the
affected implementation item; the absence of the external environment alone does not.
...
- **Changes:** Backend: Jobs/, Deployments/, Backups/, module reconciler. Frontend: queued-card, cancel, /backups form, link/icon fixes. Agent: none. DB: 3 migrations (retry scheduling; per-target exclusion/backfill; backup/restore interruption recovery and active-target uniqueness). Infra: none.
...
- **External/Live Validation Pending:** the wall-clock two-tenant demonstration and Docker-backed
  operational checks are assigned to the independent Phase 1/2 external-validation follow-up. They
  are not evidence claimed by this closeout.

## Phase 2 — Node & platform confidence — 0011-0022, 0047, 0051, 0054
...
- **External/Live Validation Pending:** fresh VPS installation, real node enrollment, live DNS/TLS,
  Docker-backed PostgreSQL, live backup/restore, live upgrade, the DR drill and the L5 live-host proof
  remain the independent **Phase 1/2 external validation** task. That task may validate the existing
  tooling; it must not be treated as unfinished repository implementation unless it finds a defect.
